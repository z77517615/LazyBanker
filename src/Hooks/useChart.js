import { incomeCategories, expenseCategories, resetCategories ,IncomeMonth,ExpenseMonth} from '../constants/categories';
...
const TransactionsPerBarChartIncome = documents.filter((t) => t.earn === title1);
const TransactionsPerBarChartExpend = documents.filter((t) => t.earn === title2);


const total = TransactionsPerDounut.reduce((acc, currVal) => acc += parseInt(currVal.amount), 0);
...
// console.log(TransactionsPerDounut)
  TransactionsPerBarChartIncome.forEach((t) => {
    const Mon = Incomemon.find((c) => c.type === t.date.slice(3,5));
    // console.log(t.date.slice(3,5))
    // console.log(Mon) 
  ...
  TransactionsPerBarChartExpend.forEach((t) => {
    const Mon = Expendmon.find((c) => c.type === t.date.slice(3,5));
    // console.log(t.date.slice(3,5))
    // console.log(Mon) 
    if (Mon) Mon.amount += parseInt(t.amount);
  });
